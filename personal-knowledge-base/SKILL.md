...
4. `INGEST`：更新原 source 页和已有 concept/entity。PDF 是原始证据；transcript 是主要 LLM 阅读层；译文是辅助层，三者共享一个 source identity。finalizer 完成质检后同步 `derived_manifest`、`derived_transcript` 和 `derived_status`；历史修复使用 `--sync-source-only`，先核验 raw SHA-256。
