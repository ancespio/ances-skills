...
wiki/derived/  PDF 的可校验转录、OCR 和译文辅助阅读层
...
- 如果论文 PDF 是主要材料：准备 2-3 份实际常见版式，说明是否需要中文全文翻译、固定术语译法，并预留解析产物和 Git LFS 所需空间。
...
把 PDF 放入 `raw/pdfs/` 后，直接说：

```text
摄入 raw/pdfs/example-paper.pdf。先转录并质检；非中文只生成中文摘要，不生成全文译文。
```

Codex 会按四阶段处理：

1. **PREPARE**：确认 raw 只读，计算 PDF SHA-256，复用或生成 source slug，检查 Python 3.10-3.12、MinerU、Docling、模型缓存、磁盘和 Git LFS。
2. **DERIVE**：优先用 MinerU 解析，失败时用 Docling 回退；先写临时目录，再规范化到最终位置。
3. **QC**：核对页数与连续页锚，抽查首/中/尾页、双栏顺序、OCR、公式、表格、参考文献和图片链接，并验证每个 artifact 的 SHA-256。
4. **INGEST**：质检通过后才创建或更新 source/concept/entity；若两个解析器都失败，记录失败并停止知识提升。
...
```text
读取 example-paper 的 transcript，定位作者对 limitation 的原文和页码。
比较原始摘要与 abstract.zh-CN.md，检查术语和否定关系。
```

完整格式和故障处理见 [pdf-derived-ingest.md](./references/pdf-derived-ingest.md)。
