...
3. **语言策略**：所有文章默认生成中文辅助摘要；中文原文只生成摘要，非中文 PDF 写入 `abstract.zh-CN.md`。全文译文会显著增加成本和审校量，必须按 transcript 篇幅分流：短篇（不超过 8,000 字符且不超过 4 页）可询问但不建议翻译；常规篇（不超过 80,000 字符且不超过 30 页）询问并建议翻译；超长篇（超过任一阈值）建议只保留摘要，只有用户明确坚持才全文翻译。
